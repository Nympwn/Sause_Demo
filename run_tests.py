...
def run_pytest(args: list):
    ...
    try:
        ...
        print('Во время выполнения тестов произошла ошибка.')
    except FileNotFoundError:
        print('Команда pytest не найдена.\n Убедитесь, что pytest установлен и доступен в PATH.')

def main_menu():
    '''Главное меню CLI'''
    while True:
        choice = questionary.select('Добро пожаловать в CLI для запуска тестов. Что вы хотите сделать?',
                                    choices=[
                                        'Запустить все тесты.',
